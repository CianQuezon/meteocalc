...
class TestDatasets:
    ...
    def get_ashrae_reference_data() -> pd.DataFrame:
        """ASHRAE Dummy values, it will be used in psychrolib calculation
           instead which is ASHRAE compliant.
        ...
        Note:
            ...
            ASHRAE requires paid subscription so these values will be used
            as reference for psychrolib wetbulb computations.
        """
        data = [
            # temp_c, rh_percent, pressure_hpa, expected_wb_c, source
            (20.0, 50.0, 1013.25, 0, "ASHRAE_DUMMY"),
            (25.0, 60.0, 1013.25, 0, "ASHRAE_DUMMY"),
            (30.0, 70.0, 1013.25, 0, "ASHRAE_DUMMY"),
            (35.0, 80.0, 1013.25, 0, "ASHRAE_DUMMY"),
            (10.0, 90.0, 1013.25, 0, "ASHRAE_DUMMY"),
            (0.0, 100.0, 1013.25, 0, "ASHRAE_DUMMY"),
            (-10.0, 85.0, 1013.25, 0, "ASHRAE_DUMMY"),
            (40.0, 30.0, 1013.25, 0, "ASHRAE_DUMMY"),
            (15.0, 45.0, 1013.25, 0, "ASHRAE_DUMMY"),
            (5.0, 75.0, 1013.25, 0, "ASHRAE_DUMMY"),
        ]
        return pd.DataFrame(
            data, 
    ...
    def get_noaa_reference_data() -> pd.DataFrame:
        ...
        data = [
            # temp_c, rh_percent, pressure_hpa, expected_wb_c, source
            (32.2, 75.0, 1013.25, 28.41, "NOAA_Weather_Calculator"),
            (37.8, 50.0, 1013.25, 28.61, "NOAA_Weather_Calculator"),
            (21.1, 80.0, 1013.25, 18.75, "NOAA_Weather_Calculator"),
            (26.7, 40.0, 1013.25, 17.65, "NOAA_Weather_Calculator"),
            (15.6, 60.0, 1013.25, 11.4, "NOAA_Weather_Calculator"),
            (4.4, 85.0, 1013.25, 3.39, "NOAA_Weather_Calculator"),
            (-1.1, 95.0, 1013.25, -1.36, "NOAA_Weather_Calculator"),
            (43.3, 25.0, 1013.25, 26.12, "NOAA_Weather_Calculator"),
        ]
        return pd.DataFrame(
            data, 
            columns=['temp_c', 'rh_percent', 'pressure_hpa', 'expected_wb_c', 'source']
    ...
    def get_extreme_conditions_data() -> pd.DataFrame:
        """Real-world extreme atmospheric conditions with calculated wet bulb values."""
        
        # Use PsychroLib to calculate wet bulb for documented extreme weather
        import psychrolib
        psychrolib.SetUnitSystem(psychrolib.SI)
        
        # Real extreme weather conditions from meteorological records
        extreme_conditions = [
            # Arctic winter (Barrow, Alaska record)
            (-20.0, 80.0, 1013.25, "Arctic_Winter_Alaska"),
            
            # Desert heat (Death Valley type)
            (47.0, 25.0, 1013.25, "Desert_Heat_Low_Humidity"),
            
            # Tropical humid extreme (Amazon/Southeast Asia)
            (35.0, 95.0, 1013.25, "Tropical_Humid_Extreme"),
            
            # High altitude cold (Tibet/Andes)
            (5.0, 20.0, 700.0, "High_Altitude_Cold_Dry"),
            
            # Dangerous heat-humidity (Persian Gulf documented)
            (40.0, 80.0, 1013.25, "Dangerous_Heat_Humidity"),
            
            # Winter storm conditions
            (-10.0, 90.0, 980.0, "Winter_Storm_Saturated"),
        ]
        
        data = []
        for temp, rh, pressure, source in extreme_conditions:
            wb = psychrolib.GetTWetBulbFromRelHum(temp, rh/100.0, pressure*100.0)
            data.append((temp, rh, pressure, wb, source))
