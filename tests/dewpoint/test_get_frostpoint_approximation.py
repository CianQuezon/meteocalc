...
class TestPsychroLibValidation:
    ...
    def test_against_psychrolib_array(self, array_conditions):
        ...
        for i, (temp_c, rh) in enumerate(zip(temps_c, rhs)):
            ...
            assert diff < 3.0, \
                f"Index {i}: difference {diff:.2f}°C too large"
...
@pytest.mark.skipif(not (HAS_METPY and HAS_PSYCHROLIB), 
...
class TestCrossReference:
    ...
    def test_all_three_agree_on_trend(self):
        ...
        tf_psychro = np.array([
            psy.GetTDewPointFromRelHum(temp_c, rh) + 273.15
            for rh in rhs
        ])
...
class TestIntegration:
    ...
    def test_consistency_with_solver(self):
        ...
        assert diff < 0.5, \
            f"Approximation differs from solver by {diff:.3f}K"
