...
class TestMagnusVectorCalculations:
    ...
    def test_vector_matches_scalar(self, magnus_water):
        ...
        # Scalar calculations
        td_scalar = np.array(
            [magnus_water.calculate(temp_k=t, rh=r) for t, r in zip(temps, rhs)]
        )
...
class TestMagnusNOAAValidation:
    ...
    def test_noaa_reference_values(
        self, magnus_water, temp_c, rh_percent, expected_td_c
    ):
...
class TestMagnusWaterVsIce:
    ...
    def test_frost_dewpoint_difference_reasonable(self):
        ...
        assert 0.3 < diff < 4.0, f"Difference ({diff:.2f}K) outside expected range"
...
class TestMagnusDeterminism:
    ...
    def test_deterministic_scalar(self, magnus_water):
        ...
        results = [magnus_water.calculate(temp_k=293.15, rh=0.6) for _ in range(10)]
