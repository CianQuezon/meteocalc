...
class TestBasicFunctionality:
    ...
    def test_scalar_input_returns_scalar(self, scalar_conditions):
        ...
        assert isinstance(td, (float, np.floating)), f"Expected float, got {type(td)}"
    ...
    def test_array_input_returns_array(self, array_conditions):
        ...
        td = Dewpoint.get_dewpoint_solver(temp_k=array_conditions["temps_k"], rh=0.6)

        assert isinstance(td, np.ndarray), f"Expected ndarray, got {type(td)}"
...
class TestMetPyValidation:
    ...
    def test_against_metpy_scalar(self, scalar_conditions):
        ...
        td_metpy_c = dewpoint_from_relative_humidity(temp_metpy, rh_metpy).magnitude
        ...
        td_metpy_c = np.array(
            [
                dewpoint_from_relative_humidity(
                    (t * units.K).to("degC"), r * 100 * units.percent
                ).magnitude
...
class TestCrossReference:
    ...
    def test_all_three_agree_on_trend(self):
        ...
        assert np.all(np.diff(td_ours) > 0), "Ours: dew point should increase with RH"
...
class TestEquationSelection:
    ...
    def test_enum_input_works(self):
        ...
        td_enum = Dewpoint.get_dewpoint_solver(
            293.15, 0.6, VaporEquationName.GOFF_GRATCH
        )
...
class TestInputValidation:
    ...
    def test_invalid_equation_raises(self):
        ...
        with pytest.raises(ValueError, match="Invalid enum"):
            Dewpoint.get_dewpoint_solver(293.15, 0.6, vapor_equation_name="invalid")
...
class TestPhysicalCorrectness:
    ...
    def test_dewpoint_below_air_temp(self):
        ...
        assert td <= temp_k, f"Dew point ({td}K) must be <= air temp ({temp_k}K)"
    ...
    def test_dew_below_frost_below_freezing(self):
        ...
        assert td < tf, f"Dew point ({td}K) should be < frost point ({tf}K)"
...
class TestIntegration:
    ...
    def test_convenience_wrapper_consistent(self):
        ...
        td_wrapper = Dewpoint.get_dewpoint(temp_k, rh, calculation_method="solver")
