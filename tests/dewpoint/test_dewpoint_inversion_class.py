...
def solver_water():
    ...
    return VaporInversionDewpoint(
        surface_type="water", vapor_equation_name="goff_gratch"
    )
...
def solver_ice():
    ...
    return VaporInversionDewpoint(surface_type="ice", vapor_equation_name="goff_gratch")
...
def solver_parametrized(request):
    ...
    return VaporInversionDewpoint(
        surface_type="water", vapor_equation_name=request.param
    )
...
class TestVaporInversionInitialization:
    ...
    def test_init_with_string_enums(self):
        ...
        assert solver.vapor_equation == VaporEquationName.GOFF_GRATCH

    def test_init_with_enum_types(self):
        """Test initialization with enum types."""
        solver = VaporInversionDewpoint(
            surface_type=SurfaceType.WATER,
            vapor_equation_name=VaporEquationName.GOFF_GRATCH,
        )
    ...
    def test_init_ice_surface(self):
        ...
        solver = VaporInversionDewpoint(
            surface_type="ice", vapor_equation_name="goff_gratch"
        )
    ...
    def test_init_different_equations(self):
        ...
        for eq in equations:
            solver = VaporInversionDewpoint(
                surface_type="water", vapor_equation_name=eq
            )
            assert solver.vapor_equation == VaporEquationName[eq.upper()]
...
class TestVaporInversionPsychroLibValidation:
    ...
    def _get_tolerance(self, temp_c, rh):
        ...
        if temp_c <= 0 and rh < 0.35:
            ...
            return 1.2
        elif temp_c <= 5 or temp_c <= 15 and rh < 0.4:
            return 1.0
        elif rh < 0.4:
            return 0.8
        else:
    ...
    def test_psychrolib_agreement(self, temp_c, rh):
        ...
        solver = VaporInversionDewpoint(
            surface_type="water", vapor_equation_name="hyland_wexler"
        )
...
class TestVaporInversionPhysicalCorrectness:
    ...
    def test_dewpoint_less_than_temperature_always(self, solver_water):
        ...
        assert_array_less(
            dewpoints,
            temps + 0.01,  # Small tolerance for numerical precision
    ...
    def test_dewpoint_equals_temperature_at_saturation(self, solver_water):
        ...
        assert_allclose(
            dewpoints, temps, atol=0.1, err_msg="At RH=100%, Td should equal T"
        )
...
class TestVaporInversionPerformance:
    ...
    def test_performance_acceptable(self, solver_water):
        ...
        # Warmup
        _ = solver_water.calculate(temp_k=np.array([293.15]), rh=np.array([0.6]))
