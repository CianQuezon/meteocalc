...
class Dewpoint:
    ...
    **Utility:**
    ...
    def get_dewpoint(
        ...
        """
        calculation_method = parse_enum(
            value=calculation_method, enum_class=CalculationMethod
        )

        if calculation_method == CalculationMethod.SOLVER:
            return Dewpoint.get_dewpoint_solver(
                temp_k=temp_k, rh=rh, vapor_equation_name=solver_vapor_equation
            )

        elif calculation_method == CalculationMethod.APPROXIMATION:
            return Dewpoint.get_dewpoint_approximation(
                temp_k=temp_k, rh=rh, dewpoint_equation_name=approximation_equation
            )
    ...
    def get_frostpoint(
        ...
        if calculation_method == CalculationMethod.SOLVER:
            return Dewpoint.get_frostpoint_solver(
                temp_k=temp_k, rh=rh, vapor_equation_name=solver_vapor_equation
            )

        elif calculation_method == CalculationMethod.APPROXIMATION:
            return Dewpoint.get_frostpoint_approximation(
                temp_k=temp_k, rh=rh, frostpoint_equation_name=approximation_equation
            )
    ...
    def get_dewpoint_solver(
        ...
        dewpoint_solver = VaporInversionDewpoint(
            surface_type="water", vapor_equation_name=vapor_equation_name
        )
    ...
    def get_frostpoint_solver(
        ...
        frostpoint_solver = VaporInversionDewpoint(
            surface_type="ice", vapor_equation_name=vapor_equation_name
        )
        return frostpoint_solver.calculate(temp_k=temp_k, rh=rh)

    @staticmethod
    def get_dewpoint_approximation(
        temp_k: Union[float, npt.ArrayLike],
        rh: Union[float, npt.ArrayLike],
        dewpoint_equation_name: Union[str, DewPointEquationName] = "magnus",
    ) -> Union[float, npt.NDArray]:
        """
        ...
        """
        dewpoint_equation_name = parse_enum(
            value=dewpoint_equation_name, enum_class=DewPointEquationName
        )

        if dewpoint_equation_name not in APPROXIMATION_DEWPOINT_REGISTRY:
            valid_approximations = [eq.value for eq in APPROXIMATION_DEWPOINT_REGISTRY]
        ...
        dewpoint_approximation_class = APPROXIMATION_DEWPOINT_REGISTRY.get(
            dewpoint_equation_name
        )
        dewpoint_approximation_equation = dewpoint_approximation_class(
            surface_type="water"
        )
    ...
    @staticmethod
    def get_frostpoint_approximation(
        temp_k: Union[float, npt.ArrayLike],
        rh: Union[npt.ArrayLike, float],
        frostpoint_equation_name: Union[str, DewPointEquationName] = "magnus",
    ) -> Union[float, npt.NDArray]:
        """
        ...
        """
        frostpoint_equation_name = parse_enum(
            value=frostpoint_equation_name, enum_class=DewPointEquationName
        )
        ...
        frostpoint_approximation_class = APPROXIMATION_DEWPOINT_REGISTRY.get(
            frostpoint_equation_name
        )
        frostpoint_approximation_equation = frostpoint_approximation_class(
            surface_type="ice"
        )
