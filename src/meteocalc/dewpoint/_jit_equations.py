...
@njit
def _buck_equation_scalar(temp_k: float, rh: float, A: float,
                             B: float, C: float) -> float:
    """
    Docstring for _dewpoint_magnus_scalar
    ...
    td_c = (C * gamma) / (A - gamma)

    return td_c + 273.15

@njit(parallel=True, fastmath=True)
def _buck_equation_vectorised(temp_k: npt.ArrayLike, rh: npt.ArrayLike, A: float,
                              B: float, C: float) -> npt.NDArray:
    """
    Docstring for _buck_equation_vectorised
    ...
    for i in prange(n):
        results[i] = _buck_equation_scalar(temp_k=temp_k[i], rh=rh[i], A=A, B=B, C=C)
    return results
